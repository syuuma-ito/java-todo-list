...
応用課題から発展させ、Java側はAPIのみを提供するように、フロントエンド側はReactで実装しました。
...
優先度は、すべてのタスクに設定したいと考えたので、通常タスクと期限付きタスクの両方に優先度を設定できるようにしました。
...
タスクリストを作り、種類の違う複数のタスクを管理できるようにしました。
...
- 全体は、シンプルでかわいい、ニューモフィズムデザインを使う (単純に自分の好みです)
...
使用したAIモデルは、GPT-5.6-solです。
Javaのコード自体は(Javaの授業なので)AIを利用せず書いています

# GitHubリポジトリ

https://github.com/syuuma-ito/java-todo-list
